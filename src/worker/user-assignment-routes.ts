...
async function canAssignToOrganization(
    ...
    if (currentUser.role === USER_ROLES.SYSTEM_ADMIN || currentUser.role === 'sys_admin') {
        return true;
    }

    // OrgAdmin pode atribuir à sua organização e subsidiárias
    if (currentUser.role === USER_ROLES.ORG_ADMIN) {
        const managedOrgId = currentUser.managed_organization_id;
...
userAssignmentRoutes.delete("/:assignmentId", demoAuthMiddleware, async (c) => {
    ...
    `).bind(assignmentId).first() as any;
        ...
        await db.prepare(`
      UPDATE user_organizations SET is_active = 0, updated_at = datetime('now') WHERE id = ?
