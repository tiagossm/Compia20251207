...
checklistRoutes.get("/checklist-templates", demoAuthMiddleware, async (c) => {
  ...
  try {
    ...
    `;
    let params: any[] = [];
    let whereClause = ["ct.is_category_folder = false OR ct.is_category_folder IS NULL"]; // Excluir pastas antigas
...
checklistRoutes.put("/checklist-templates/:id", demoAuthMiddleware, async (c) => {
  ...
  try {
    ...
    await env.DB.prepare(`
      ...
      SET name = ?, description = ?, category = ?, is_public = ?, folder_id = ?, updated_at = datetime('now')
      WHERE id = ?
...
checklistRoutes.post("/checklist-templates/generate-ai-simple", demoAuthMiddleware, async (c) => {
  ...
  try {
    ...
    if (!env.OPENAI_API_KEY) {
      console.error('[AI-CHECKLIST] OpenAI API key não configurada');
      return c.json({
        success: false,
  ...
  try {
    ...
    const result = await env.DB.prepare(`
      INSERT INTO action_items (
        ...
        why_reason, how_method, who_responsible, when_deadline, how_much_cost,
        status, priority, is_ai_generated, assigned_to, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, false, ?, datetime('now'), datetime('now'))
    `).bind(
      inspectionId,
