...
export async function ensureSystemAdminAccess(env: Env): Promise<void> {
  ...
  try {
    ...
    const SYSTEM_ADMIN_ID = '01990d69-5246-738d-8605-1ed319a3f98d'; // ID real do usuário
    const DEMO_USER_EMAIL = 'demo@compia.test';
    const DEMO_USER_ID = 'demo-user-01990d69-5246-738d-8605-1ed319a3f98d'; // ID do usuário demo
    ...
    if (systemAdmin.organization_id !== masterOrg.id) {
      ...
      await env.DB.prepare(`
        UPDATE users 
        SET organization_id = ?, managed_organization_id = ?, updated_at = datetime('now')
        WHERE id = ?
