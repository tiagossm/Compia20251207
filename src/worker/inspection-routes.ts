...
import { TenantContext } from "./tenant-auth-middleware";
...
inspectionRoutes.patch("/:id/responses/:itemId", demoAuthMiddleware, async (c) => {
  ...
  try {
    ...
    await env.DB.prepare(`
      ...
      SET field_responses = ?, is_compliant = ?, updated_at = datetime('now')
...
inspectionRoutes.post("/:id/signatures", demoAuthMiddleware, async (c) => {
  ...
  try {
    ...
    await env.DB.prepare(`
      UPDATE inspections 
      SET inspector_signature = ?, responsible_signature = ?, responsible_name = ?, updated_at = datetime('now')
      WHERE id = ?
    `).bind(
      inspector_signature || null,
      responsible_signature || null,
...
inspectionRoutes.delete("/:id", demoAuthMiddleware, async (c) => {
  ...
  try {
    ...
    // Excluir itens relacionados primeiro (CASCADE via FK não funciona em SQLite local)
    ...
    await env.DB.prepare('DELETE FROM inspection_reports WHERE inspection_id = ?').bind(inspectionId).run();
    await env.DB.prepare('DELETE FROM inspection_collaborators WHERE inspection_id = ?').bind(inspectionId).run();
...
Base sua decisão exclusivamente nas evidências analisadas e seja específico sobre os achados visuais/sonoros.`
    ...
    if (actionPlan.requires_action) {
      ...
      await env.DB.prepare(`
        UPDATE inspection_items 
        SET ai_action_plan = ?, updated_at = datetime('now')
        WHERE id = ?
