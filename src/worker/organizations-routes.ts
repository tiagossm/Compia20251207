...
app.get('/api/organizations/stats', demoAuthMiddleware, async (c) => {
  try {
...
app.get('/api/organizations', demoAuthMiddleware, async (c) => {
  try {
    const user = c.get('user') as ExtendedMochaUser;
    const db = getDatabase(c.env);
...
app.get('/api/organizations/:id', demoAuthMiddleware, async (c) => {
  try {
    const user = c.get('user') as ExtendedMochaUser;
    const db = getDatabase(c.env);
...
app.post('/api/organizations', demoAuthMiddleware, async (c) => {
  try {
    const user = c.get('user') as ExtendedMochaUser;
    const db = getDatabase(c.env);
...
VALUES(?, ?, ?, ?, ?, ?, datetime('now'))
    ...
    return c.json({ error: 'Erro ao criar organização' }, 500);
  }
});

// Update organization
app.put('/api/organizations/:id', demoAuthMiddleware, async (c) => {
  try {
    const user = c.get('user') as ExtendedMochaUser;
    const db = getDatabase(c.env);
    const organizationId = parseInt(c.req.param('id'));
...
// Delete organization
app.delete('/api/organizations/:id', demoAuthMiddleware, async (c) => {
  try {
    const user = c.get('user') as ExtendedMochaUser;
    const db = getDatabase(c.env);
    const organizationId = parseInt(c.req.param('id'));
