...
import { requireProtectedSysAdmin } from "./rbac-middleware";
import { autoFixSystemAdmin } from "./system-admin-protection";
...
systemAdminRoutes.post("/ensure-protected-sysadmin", authMiddleware, requireProtectedSysAdmin(), async (c) => {
  ...
  try {
    ...
    `).bind(
      ...
      '01990d69-5246-738d-8605-1ed319a3f98d'
    ).run();
  ...
  try {
    ...
    const securityLogs = await env.DB.prepare(`
      ...
      WHERE action_type IN ('system_security_check', 'user_update_blocked', 'user_delete_blocked')
         ...
         OR target_id = '01990d69-5246-738d-8605-1ed319a3f98d'
    ...
    return c.json({
      ...
      protected_user_id: '01990d69-5246-738d-8605-1ed319a3f98d',
...
systemAdminRoutes.get("/protection-status", authMiddleware, requireProtectedSysAdmin(), async (c) => {
  ...
  try {
    ...
    const protectedUser = await env.DB.prepare(`
      ...
      FROM users 
      WHERE email = ? OR id = ?
    `).bind('eng.tiagosm@gmail.com', '01990d69-5246-738d-8605-1ed319a3f98d').first() as any;
    ...
    `).bind(USER_ROLES.SYSTEM_ADMIN, 'eng.tiagosm@gmail.com', '01990d69-5246-738d-8605-1ed319a3f98d').first() as any;
    ...
    return c.json({
      ...
      system_status: {
        ...
        protected_id: '01990d69-5246-738d-8605-1ed319a3f98d'
      },
      security_measures: {
...
systemAdminRoutes.post("/force-fix-protected-user", authMiddleware, requireProtectedSysAdmin(), async (c) => {
  ...
  try {
    ...
    if (!protectedUser) {
      ...
      await env.DB.prepare(`
        UPDATE users 
        SET role = ?, can_manage_users = ?, can_create_organizations = ?, is_active = ?, updated_at = datetime('now')
        WHERE email = ? OR id = ?
      `).bind(
        USER_ROLES.SYSTEM_ADMIN,
        true,
